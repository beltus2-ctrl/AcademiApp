...
const MESSAGES_MOTIVATION = [
  ...
  'Votre savoir est un cadeau inestimable ! 💎',
  'Merci d etre present pour vos etudiants ! 🙏',
];

const cartes: CarteProfesseur[] = [
  {
    icone: '💬',
    label: 'Chats',
    code: 'CHAT_QUEUE',
    description: 'Conversations et demandes etudiants',
    accent: '#4A90D9',
    indicateur: 'LIVE',
    route: '/prof/chats',
  },
  {
    ...
    label: 'Appels recus',
    code: 'CALL_RADAR',
    description: 'Signalements urgents et relais direct',
    accent: '#4CAF50',
    indicateur: 'READY',
    route: '/prof/chats',
  },
  {
    icone: '📝',
    label: 'Salon_Corrections',
    code: 'COPY_REVIEW',
    description: 'Creer un salon privé - partager le code avec vos etudiants',
    accent: '#FF7043',
    indicateur: 'CHECK',
    route: '/prof/salon-corrections',
  },
  {
    icone: '📊',
...
export default function TableauProfesseur() {
  ...
  const [stats, setStats] = useState<StatEtudiant>({ total: 0, actifs: 0 });
  const [messageMotivation] = useState(
    MESSAGES_MOTIVATION[Math.floor(Math.random() * MESSAGES_MOTIVATION.length)]
  );
  ...
  useEffect(() => {
    chargerProfil();
    chargerStats();
    const unsubscribe = ecouterAppels();
    return unsubscribe;
  }, []);
  ...
  useEffect(() => {
    if (appels.length === 0) {
      appelPulseAnim.setValue(1);
      return;
    }

    const animation = Animated.loop(
      Animated.sequence([
        Animated.timing(appelPulseAnim, { toValue: 1.15, duration: 300, useNativeDriver: true }),
        Animated.timing(appelPulseAnim, { toValue: 1, duration: 300, useNativeDriver: true }),
        Animated.timing(appelPulseAnim, { toValue: 1.15, duration: 300, useNativeDriver: true }),
        Animated.timing(appelPulseAnim, { toValue: 1, duration: 1000, useNativeDriver: true }),
      ])
    );
    animation.start();

    return () => animation.stop();
  }, [appelPulseAnim, appels.length]);
  ...
  const ecouterAppels = () => {
    const q = query(collection(db, 'appels'), where('statut', '==', 'en_attente'));
  ...
  const chargerStats = async () => {
    try {
      ...
      setStats({ total: snap.size, actifs: Math.floor(snap.size * 0.7) });
    } catch {}
  };

  const toggleDisponibilite = async (valeur: boolean) => {
    setDisponible(valeur);
    const utilisateur = auth.currentUser;
    if (!utilisateur) return;

    try {
      await updateDoc(doc(db, 'utilisateurs', utilisateur.uid), { disponible: valeur });
    } catch {
      setDisponible(!valeur);
    }
  };
  ...
  const indicateurCarte = (carte: CarteProfesseur) => {
    ...
    if (carte.code === 'CHAT_QUEUE') return disponible ? 'ONLINE' : 'OFFLINE';
    return carte.indicateur;
  };

  const ouvrirCarte = (carte: CarteProfesseur, index: number) => {
    Animated.sequence([
  ...
  const accepterAppel = async (appel: Appel) => {
    try {
      await updateDoc(doc(db, 'appels', appel.id), { statut: 'accepte' });
      router.push('/chat/professeur' as any);
    } catch {}
  };
  ...
  const seDeconnecter = async () => {
    Alert.alert('Deconnexion', 'Voulez-vous vous deconnecter ?', [
      ...
      { text: 'Deconnecter', style: 'destructive', onPress: confirmerDeconnexion },
    ]);
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <Text style={styles.titre}>PROF_CONTROL</Text>
          <Text style={styles.sousTitre}>Bienvenue {nom || 'Professeur'} • Supervision IUT Douala</Text>

          <View style={styles.statusRow}>
            <View style={styles.statusPill}>
              <View style={[styles.statusDot, { backgroundColor: disponible ? '#4CAF50' : '#FF5252' }]} />
              <Text style={styles.statusTexte}>{disponible ? 'DISPONIBLE' : 'INDISPONIBLE'}</Text>
            </View>
            <View style={styles.statusPill}>
              <View style={[styles.statusDot, { backgroundColor: '#FFC107' }]} />
              <Text style={styles.statusTexte}>{stats.actifs}/{stats.total} ETUDIANTS</Text>
            </View>
          </View>
        </Animated.View>
        ...
        >
          <View style={styles.panelHeader}>
            ...
            <Text style={styles.panelHeaderTitre}>ACADEMIAPP_PROFESSOR_CONSOLE.exe</Text>
          </View>
          ...
          <View style={styles.panelBody}>
            ...
            <View style={styles.commandBand}>
              <Text style={styles.commandTitre}>[ CENTRE_DE_SUPERVISION ]</Text>
              <Text style={styles.commandTexte}>Chats, appels, corrections et progression etudiante.</Text>
              <Text style={styles.commandTexte}>{messageMotivation}</Text>
            </View>
            ...
            <View
              style={[
                ...
                {
                  borderColor: disponible ? 'rgba(76,175,80,0.5)' : 'rgba(255,82,82,0.3)',
                  backgroundColor: disponible ? 'rgba(76,175,80,0.08)' : 'rgba(255,82,82,0.06)',
                },
              ]}
            >
              <View style={styles.disponibiliteGauche}>
                <Animated.View
                  style={[
                    styles.disponibilitePoint,
                    {
                      backgroundColor: disponible ? '#4CAF50' : '#FF5252',
                      transform: [{ scale: disponible ? pulseAnim : 1 }],
                    },
                  ]}
                />
                <View style={styles.disponibiliteTexteBloc}>
                  <Text style={styles.disponibiliteTitre}>
                    {disponible ? 'Disponible' : 'Indisponible'}
                  </Text>
                  <Text style={styles.disponibiliteTexte}>
                    {disponible
                      ? 'Les etudiants peuvent vous contacter'
              ...
              <Switch
                ...
                trackColor={{ false: '#FF525233', true: '#4CAF5033' }}
                thumbColor={disponible ? '#4CAF50' : '#FF5252'}
              />
            </View>

            {appels.length > 0 && (
              <View style={styles.appelsContainer}>
                <View style={styles.appelsSectionTitre}>
                  <Animated.View style={[styles.appelsIcone, { transform: [{ scale: appelPulseAnim }] }]}>
                    <Text style={styles.appelsIconeTexte}>📞</Text>
                  </Animated.View>
                  <Text style={styles.appelsTitre}>{appels.length} appel(s) en attente</Text>
                </View>
                ...
                {appels.map(appel => (
                  <View key={appel.id} style={styles.appelCard}>
                    ...
                    <View style={styles.appelInfos}>
                      <Text style={styles.appelNom}>{appel.etudiantNom || 'Etudiant'}</Text>
                      <Text style={styles.appelTemps}>Appel en attente...</Text>
                    </View>
                    <View style={styles.appelActions}>
                      <TouchableOpacity
                        style={styles.boutonAccepter}
                        onPress={() => accepterAppel(appel)}
                        activeOpacity={0.82}
                      >
                        <Text style={styles.boutonAccepterTexte}>✓</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                  ...
                  <TouchableOpacity activeOpacity={0.86} onPress={() => ouvrirCarte(carte, index)}>
                    ...
                    >
                      ...
                      <View style={styles.carteFooter}>
                        ...
                        <Text style={styles.carteAction}>{indicateurCarte(carte)}</Text>
                      </View>
                    </LinearGradient>
                  </TouchableOpacity>
                </Animated.View>
            ...
            <View style={styles.securiteContainer}>
              ...
              <View style={styles.securiteItem}>
                ...
                <Text style={styles.securiteTexte}>PROF SECURE</Text>
              </View>
            </View>
          ...
          <View style={styles.panelFooter}>
            <Text style={styles.panelFooterTexte}>🔒 AcademiApp © 2026 • Console professeur • IUT Douala</Text>
          </View>
        </Animated.View>

        <Animated.View style={[styles.sysStats, { opacity: fadeAnim }]}>
          {[
            { label: 'ETUDIANTS', valeur: stats.total.toString(), icone: '👥' },
            { label: 'APPELS', valeur: appels.length.toString(), icone: '📞' },
            { label: 'DISPO', valeur: disponible ? 'OUI' : 'NON', icone: disponible ? '🟢' : '🔴' },
          ].map(stat => (
...
const styles = StyleSheet.create({
  ...
  disponibiliteCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: 12, padding: 12, borderWidth: 1.5, gap: 12 },
  disponibiliteGauche: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10 },
  disponibilitePoint: { width: 10, height: 10, borderRadius: 5 },
  disponibiliteTexteBloc: { flex: 1 },
  disponibiliteTitre: { color: '#FFFFFF', fontSize: 13, fontWeight: '900' },
  disponibiliteTexte: { color: '#8BA4C4', fontSize: 10, lineHeight: 15 },
  appelsContainer: { backgroundColor: 'rgba(255,82,82,0.06)', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: 'rgba(255,82,82,0.22)', gap: 10 },
  appelsSectionTitre: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  appelsIcone: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,82,82,0.18)', alignItems: 'center', justifyContent: 'center' },
  appelsIconeTexte: { fontSize: 18 },
  appelsTitre: { color: '#FF6B6B', fontSize: 12, fontWeight: '900', letterSpacing: 0.7, fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace' },
  appelCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(0,8,20,0.35)', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: 'rgba(255,82,82,0.2)', gap: 10 },
  appelAvatar: { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(74,144,217,0.2)', alignItems: 'center', justifyContent: 'center' },
  appelAvatarTexte: { color: '#4A90D9', fontSize: 11, fontWeight: '900' },
  appelInfos: { flex: 1 },
  appelNom: { color: '#FFFFFF', fontSize: 12, fontWeight: '800' },
  appelTemps: { color: '#8BA4C4', fontSize: 10 },
  appelActions: { flexDirection: 'row', gap: 8 },
  boutonAccepter: { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(76,175,80,0.18)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(76,175,80,0.4)' },
  boutonAccepterTexte: { color: '#4CAF50', fontSize: 15, fontWeight: '900' },
  boutonRefuser: { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(255,82,82,0.18)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,82,82,0.4)' },
  boutonRefuserTexte: { color: '#FF5252', fontSize: 15, fontWeight: '900' },
